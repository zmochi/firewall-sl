...
int main(void) {
  ...
  // queue_map_fd = bpf_map__fd(skel->maps.preprocess_pkts);
  // pkt_dec_fd = bpf_map__fd(skel->maps.pkt_decision);

  // if (queue_map_fd < 0) {
  //   ERR("Couldn't get queue map fd");
  //   goto cleanup_load;
  // }
  //
  // if (pkt_dec_fd < 0) {
  //   ERR("Couldn't get packet decisions map fd");
  ...
  while (true) {
    ...
    err =
        bpf_map__update_elem(pkt_dec_map, &pkt.pkt_dec_map_index,
                             sizeof(pkt.pkt_dec_map_index), &dc, sizeof(dc), 0);
    if (err < 0) {
